...
class ParallaxRenderer extends CanvasImageLayerRenderer {
	 ...
	 */
	getData(pixel) {
		const x = pixel[0] * this.scale[0] + this.offset[0];
		const y = pixel[1] * this.scale[1] + this.offset[1];
		super.getData([x, y]);
	}
   ...
   */
	renderFrame(frameState, target) {
		...
		if (render && dw >= 0.5 && dh >= 0.5) {
		  const dx = transform[4] * this.scale[0] + this.offset[0];
		  const dy = transform[5] * this.scale[1] + this.offset[1];
